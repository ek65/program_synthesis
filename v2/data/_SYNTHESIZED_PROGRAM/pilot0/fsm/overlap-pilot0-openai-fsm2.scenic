...
behavior CoachBehavior():
    ...
    do Idle() until lambda_precondition_pass()
    # CHANGED: Now immediately StopAndReceiveBall() here instead of calling for a pass
    do Speak("Wait to receive the ball from teammate")
    do StopAndReceiveBall()
    do Speak("Wait until you have received the ball from teammate")
    do Idle() until lambda_precondition_Coach_has_ball()
    if lambda_precondition_shot():
        do Speak("Path to goal is clear and you have the ball; shoot at goal")
        do Shoot(goal)
    elif lambda_precondition_teammate_shot():
        do Speak("No clear shot to goal, but path to teammate is open; pass to teammate")
        do Pass(teammate)
    else:
        do Speak("No clear path to shoot or pass, stop and receive ball, take possession")
        do StopAndReceiveBall()
    do Idle()
