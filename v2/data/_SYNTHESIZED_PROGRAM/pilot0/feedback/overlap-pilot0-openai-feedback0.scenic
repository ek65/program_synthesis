...
# Constraint instantiations (no change for these, here for reference)
A1_overlap = Overlap({'player': 'Coach', 'ball': 'ball', 'goal': 'goal', 'opponent': 'opponent', 'theta': {'avg': 40, 'std': 6}, 'dist': {'avg': 5, 'std': 1}})
...
# Added for "move up the field by 4m"
A1_coach_move_up_4m = HeightRelation({'obj': 'Coach', 'relation': 'above', 'ref': None, 'height_threshold': {'avg': 4, 'std': 0.5}})

# Target for Coach overlapping on side, receiving ball, and moving up by 4m
def λ_target_overlap():
    # Combine Overlap and HeightRelation using conjunction
    overlap_and_upfield = A1_overlap & A1_coach_move_up_4m
    return overlap_and_upfield.dist(simulation(), ego=True)
...
behavior CoachBehavior():
    do Idle() for 3 seconds  # ensure start structuring
    do Speak("Move to overlap position: 40 deg angle from ball, 5m from ball, and move up the field by 4m.")  # narration updated per feedback
    do MoveTo(λ_target_overlap(), True)
    do Speak("Wait until you get possession of the ball from the pass.")
    do Idle() until λ_precondition_coach_possession()
    # CHANGED: As soon as coach has the ball, immediately pass back to teammate
    do Speak("Now that you have the ball, pass it back to your teammate right away.")
    do Pass(teammate)
    do Idle()  # always end with Idle()
