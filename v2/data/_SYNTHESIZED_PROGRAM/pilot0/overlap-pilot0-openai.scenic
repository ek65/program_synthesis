...
behavior CoachBehavior():
    ...
    do Speak("Move overlapping teammate: 38 degree angle to ball-goal, 6 meters from ball.")
    do MoveTo(λ_target_overlap(), True)
    do Speak("Wait until you are close enough to receive the pass, within 2 meters.")
    do Idle() until λ_target_receive1()
    do Speak("Stop and receive the ball after your teammate passes it to you.")
    do StopAndReceiveBall()
    do Speak("Wait until you have ball possession to decide your next move.")
    do Idle() until λ_precondition_has_possession()
    if λ_precondition_HasPath_Coach_goal():
        do Speak("There is a clear 2 meter wide path, average, for you to shoot at the goal.")
        do Speak("Shoot to the goal.")
        do Shoot(goal)
        do Idle()
    else:
        do Speak("No clear path to shoot. Pass to teammate for a shot instead.")
        do Pass(teammate)
        do Idle()
